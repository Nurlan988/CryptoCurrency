    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            <label for="wallet" class="block text-sm font-medium text-gray-700"
              >Ticker</label
            >
            <div class="elative rmt-1 rounded-md shadow-md">
              <input
                type="text"
                name="wallet"
                id="wallet"
                class="block w-full pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
                placeholder="Exemple DOGE"
                v-model.trim="ticker"
                @keydown.enter="add"
                @input="inputTicker"
              />
            ...
            <div v-if="isRepeatTicker" class="text-sm text-red-600">
      ...
      <template v-if="tickers">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
            v-for="(t, index) in filtredTickers()"
            :key="index"
            @click="select(t)"
      ...
      <template v-if="selectedTicker">
        ...
        <section class="relative">
          <h3
            class="text-lg leading-6 font-medium text-gray-900 my-8 uppercase"
          >
          ...
          >
            <div
              v-for="(bar, index) in normolizedGraph"
              :key="index"
              :style="{ height: `${bar}%` }"
...
export default {
  ...
  mounted() {
    this.calcMaxGraphElement;
    // window.addEventListner("resize", this.calcMaxGraphElement);
  },
  beforeUnmount() {
    // window.removeEventListner("resize", this.calcMaxGraphElement);
  },
  computed: {
    normolizedGraph() {
  ...
  methods: {
    ...
    inputTicker(tickerName) {
      if (this.ticker !== tickerName) {
        this.isRepeatTicker = false;
      }
      if (this.ticker !== null) {
    ...
    add() {
      if (this.ticker === null || this.ticker === "") {
        ...
        const currentTicker = {
          name: (this.ticker = this.ticker.toUpperCase()),
          price: "-",
        };
        this.tickers.forEach((t) => {
          if (this.ticker === t.name) {
            this.isRepeatTicker = true;
          }
        });
    ...
    hundleDalete(tickerToRemote) {
      this.tickers = this.tickers.filter((t) => t !== tickerToRemote);
      if (this.selectedTicker === tickerToRemote) {
        this.selectedTicker = null;
      }
      unsubscribeFromTicker(tickerToRemote.name);
    },
    closeGraph() {
      this.selectedTicker = null;
    },
